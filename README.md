...
- **球员库**：Excel/CSV 批量导入（固定两列：球员ID | 外文名），覆盖更新；球员具有经纪人档位（温和/普通/苛刻）属性
- **工资谈判**：设定违约金 → 生成预期工资 → 最多 3 次递增报价；成功签约，3 次失败自动按预期工资成约；报价失败时反馈把握分档（四档）；报价过低存在直接失败风险（触发后谈判立即终止并按该次预期结算）
...
另有工资计算相关参数（键名见 `/谈判查看配置`）与输入校验边界（CA/PA/年龄/违约金上限）等配置项。内部参数在 `/谈判查看配置` 中隐藏显示（显示为「（内部参数，已隐藏）」）。
